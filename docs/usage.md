# nf-core/pgscalc: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/pgscalc/usage](https://nf-co.re/pgscalc/usage)
...
```console
nextflow run nf-core/pgscalc --input samplesheet.csv --genome GRCh37 -profile docker
```
...
First, go to the [nf-core/pgscalc releases page](https://github.com/nf-core/pgscalc/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
