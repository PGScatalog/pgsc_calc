...
* The distribution of PGS scores can differ between genetic ancestry groups, but this does not necessarily correspond to differences in risk (e.g., disease prevalence or mean biomarker values). These differences arise from changes in allele frequencies and linkage disequilibrium (LD) patterns. `We recommend using the ancestry normalisation feature to address this confounding <https://pgsc-calc.readthedocs.io/en/latest/explanation/geneticancestry.html>`_.
...
.. note:: **Interpretation**
   ...
   * Interpretation should always consider population context, phenotype definition, and validation performance.
   ...
   See Abu-El-Hija *et al.* in :ref:`further-reading` for more details.
...
Each ``pgsc_calc`` run produces a summary HTML report (``report.html``) containing:
...
.. note:: **Interpretation**
   ...
   * Performance metrics (e.g., R² or AUC) when available.
...
* Use ancestry-normalised or Z-score outputs when applying PGS to individuals of diverse ancestries.
* Interpret scores within the population context; avoid making absolute statements about risk.
* When using population descriptors, use language describing similarity to reference genetic ancestry groups, e.g. *1KG-EUR-like* (See Box 5-3, `NASEM, 2023 <#further-reading>`_).
...
* Abu-El-Haija *et al.* (2023) *The clinical application of polygenic risk scores: A points to consider statement of the American College of Medical Genetics and Genomics (ACMG)*. Genet Med. doi: https://doi.org/10.1016/j.gim.2023.100803
* Lambert, S.A. *et al.* (2024) *Enhancing the Polygenic Score Catalog with tools for score calculation and ancestry normalization*. https://doi.org/10.1038/s41588-024-01937-x
* Lennon *et al.* (2024) *Selection, optimization and validation of ten chronic disease polygenic risk scores for clinical implementation in diverse US populations*. Nat Med. https://doi.org/10.1038/s41591-024-02796-z
...
* NASEM (2023) *Using Population Descriptors in Genetics and Genomics Research: A New Framework for an Evolving Field*. The National Academies Press. https://doi.org/10.17226/26902
* Wand *et al.* (2021) *Improving reporting standards for polygenic scores in risk prediction studies*. Nature. https://doi.org/10.1038/s41586-021-03243-6
