...
# check mandatory report parameters
required <- c("sampleset", "score_path", "match_log_path", "version")

# Identify missing or empty parameters
missing <- required[sapply(params[required], function(x) is.null(x) || x == "")]
  ...
  return(
    log %>%
      mutate(
        ...
        is_ambiguous = factor(is_ambiguous, levels = c("false", "true"))
      )
  )
}
log_df <- read_log(params$match_log_path)
...
```{r, echo=FALSE}
message(stringr::str_interp("pgscatalog/pgsc_calc ${params$version}"))
``` 
...
```{r load_scorefiles}
json_list <- jsonlite::read_json(params$log_scorefiles)
...
```{r setup_matches}
log_df %>%
  mutate(match_summary = forcats::fct_collapse(match_summary, matched = "matched", other_level = "unmatched")) %>%
  group_by(sampleset, accession, match_summary, is_match_rate_ok) %>%
  count(wt = count) %>%
  group_by(sampleset, accession) %>%
  mutate(percent = round(n / sum(n) * 100, 1), n_variants = sum(n)) %>%
  arrange(accession, desc(percent)) %>%
  tidyr::pivot_wider(names_from = match_summary, values_from = c(n, percent)) %>%
...
compat %>%
  select(sampleset, accession, n_variants, is_match_rate_ok, percent_matched,
         n_matched, n_unmatched) %>%
  mutate(is_match_rate_ok = as.logical(is_match_rate_ok)) %>%
  DT::datatable(rownames = FALSE,
    ...
    colnames = c(
      ...
      "Passed matching" = "is_match_rate_ok",
...
if(params$run_ancestry == TRUE){
    ...
    log_df %>%
      group_by(sampleset, accession) %>%
      count(match_summary, is_multiallelic, is_ambiguous, match_flipped, wt=count) %>%
      mutate(percent = round(n / sum(n) * 100, 2),
             match_summary = forcats::fct_relevel(match_summary, "matched", "excluded", "unmatched")) %>%
      arrange(accession, match_summary) %>%
      mutate(accession = stringr::str_replace_all(accession, "_", " ")) %>%
      DT::datatable(rownames=FALSE,
                    extensions = 'Buttons',
        options = list(dom = 'Bfrtip',
...
scores <- readr::read_csv(params$score_path) 
n_scores <- length(unique(scores$accession))
n_samples <- length(unique(scores$sample_id))
...
uscores <- unique(scores$accession)
...
for(current_pgs in uscores_plot){
  long_scores <- scores %>% 
    select(!percentile_MostSimilarPop) %>% 
    filter(PGS == current_pgs) %>% 
    gather(Method, score, -sampleset, -FID, -IID, -PGS)
  long_scores %>%
...
scores %>%
  ungroup() %>%
  select(sample_id, sampleset, accession, score) %>%
  # extract PGS Catalog identifier
  mutate(PGS = ifelse(
...
```{r, eval=!params$run_ancestry, echo=FALSE}
stringr::str_glue("results/scores.txt.gz")
```
