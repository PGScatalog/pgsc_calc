...
if (length(scorefiles) == 1) {
  ...
  scores <-
    readr::read_tsv(scorefiles[1], show_col_types = FALSE) %>%
    rename(IID = `#IID`)
  scores$sampleset <- get_sampleset_name(scorefiles[1])

  for(path in scorefiles[2:length(scorefiles)]){
    message(paste("Reading next scorefile:", path))
    df2 <- readr::read_tsv(path, show_col_types = FALSE) %>%
      rename(IID = `#IID`)
    df2$sampleset <- get_sampleset_name(path)
    message("Aggregating")
    dplyr::bind_rows(scores, df2) %>%
