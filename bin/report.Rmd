...
read_and_aggregate <- function(df, path) {
  sampleset <- get_sampleset_name(path)
  
  if (is.null(nrow(df))) {
    # initialise 
    message("Reading first scorefile")
    df <- readr::read_tsv(path, show_col_types = FALSE) %>%
      rename(IID = `#IID`)
    df$sampleset <- sampleset
    return(df)
  } else {
    message("Reading next scorefile")
    df2 <- readr::read_tsv(path, show_col_types = FALSE) %>%
      rename(IID = `#IID`)
    df2$sampleset <- sampleset
    
    message("Aggregating")
    dplyr::bind_rows(df, df2) %>%
      group_by(sampleset, IID) %>%
      summarise(across(matches("SUM$"), sum, na.rm = TRUE),
                DENOM_SUM = sum(DENOM, na.rm = TRUE)) %>%
      return(.)
  }
}
...
if (length(scorefiles) == 1) {
  ...
  scores <- Reduce(read_and_aggregate, scorefiles) %>%
    ungroup() %>%
    mutate(across(matches("SUM$"), ~ .x / DENOM_SUM, .names = "{.col}_AVG"))
}
